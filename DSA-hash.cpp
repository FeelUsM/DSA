...
int main()
{
	...
	cout<< "проверка модуля Р: G^Q=1(mod P)";
	step_mod(PP,GG,len2,QQ,len2,X3);
	cout<<"\nG^Q = "<<hex_mas(len2,X3)<<endl;

	//cout <<endl << "Секретный ключ k "<<endl
	//	<<hex_mas(len2,k)<<endl;
	mod_p(XX,len2,QQ,len2);
	cout<< "Секретный ключ x, приведенный по модулю Р"<<endl
		<<hex_mas(len2,XX)<<endl;
	mod_p(k,len2,QQ,len2);
	cout<< "Секретный ключ k, приведенный по модулю Р"<<endl
		<<hex_mas(len2,k)<<endl;
	...
	cout<<endl<<"Введите хеш"<<endl;
	cin>>hex_mas(10,HH);
	cout<<"введено: "<<hex_mas(10,HH)<<endl;
